...
static void fs_sync_mmcblk0_log(void)
{
	...
	if (time1 - fs_sync_last_t >= (unsigned long long)SYNC_PRT_TIME_PERIOD)
	{
		...
		for(i=0;i<ID_CNT;i++)
		{
			...
			{
				sprintf(xlog_buf2+31+i*9, "(%4d):%d ", fs_sync[i].pid, fs_sync[i].cnt);	//20=strlen("Sync [(PID):cnt] -- "), 9=strlen("(%4d):%d ")
				ptr_flag = true;
			}
		}	
		if(ptr_flag)
		{		
			...
			xlog_printk(ANDROID_LOG_DEBUG, "BLOCK_TAG", "%s", xlog_buf2);			
		}		
	...
	do{
		...
		for(i=0;i<ID_CNT;i++)
		{
			if (curr_pid == fs_sync[i].pid) 	//found in the array
			{
				fs_sync[i].cnt++;
				fs_idx =i;
				break;
			}
			else if (fs_sync[i+1].pid== 0)	//find the empty space
			{
...
static void fs_sync_mmcblk1_log(void)
{
	...
	if (time1 - fs1_sync_last_t >= (unsigned long long)SYNC_PRT_TIME_PERIOD)
	{
		...
		for(i=0;i<ID_CNT;i++)
		{
			...
			{
				sprintf(xlog_buf3+31+i*9, "(%4d):%d ", fs1_sync[i].pid, fs1_sync[i].cnt);	//20=strlen("Sync [(PID):cnt] -- "), 9=strlen("(%4d):%d ")
				ptr_flag = true;
			}
		}	
		if(ptr_flag)
		{		
			...
			xlog_printk(ANDROID_LOG_DEBUG, "BLOCK_TAG", "%s", xlog_buf3);			
		}		
		for (i=0;i<ID_CNT;i++)	//clear
		{
			fs1_sync[i].pid=0;
	...
	do{
		...
		for(i=0;i<ID_CNT;i++)
		{
			if (curr_pid == fs1_sync[i].pid) 	//found in the array
			{
				fs1_sync[i].cnt++;
				fs1_idx =i;
				break;
			}
			else if (fs1_sync[i+1].pid== 0)	//find the empty space
			{
				if(i+1==ID_CNT)	//check buf full, record in the last element
				{
					i -= 1;
