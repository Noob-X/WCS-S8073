		...
		const struct usb_ctrlrequest *ctrl, unsigned number)
{
	...
	USB_LOGGER(SET_CONFIG, SET_CONFIG, gadget->speed, number, \
		c ? c->label : "unconfigured");
		...
		int (*bind)(struct usb_configuration *))
{
	...
	if (status < 0) {
		...
		xlog_printk(ANDROID_LOG_DEBUG, "USB20", "%s: bind fialed and the list should be init because there is one entry only");
		//ALPS00235316 and ALPS00234976
		config->cdev = NULL;
	} else {
		unsigned	i;
			      ...
			      struct usb_configuration *config)
{
	...
	//ALPS00235316 and ALPS00234976
	//check the usb_add_config function, if the android_bind_config failed, the config->list entry has been delete
	if(config->cdev != NULL)
	{
	//ALPS00235316 and ALPS00234976
	...
	{
		xlog_printk(ANDROID_LOG_DEBUG, "USB20", "%s: config->list has been delete!! \n", __func__);
	}
	//ALPS00235316 and ALPS00234976
...
static void composite_disconnect(struct usb_gadget *gadget)
{
	...
	{
		xlog_printk(ANDROID_LOG_DEBUG, "USB20", "%s:  reassign the complete function!!\n", __func__);
