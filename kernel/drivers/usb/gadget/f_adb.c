...
OPEN_END: 	
	...
	return ret ;
#else
	printk(KERN_INFO "%s %s %d open_excl(%d): %p check adb_release %p \n",
		__FILE__, __func__, __LINE__, atomic_read(&_adb_dev->open_excl), adb_fops.open, adb_fops.release);

	if (!_adb_dev)
		return -ENODEV;

	/*Workaround for being unable to call adb_release from adbd */
	while (open_release_pair > 0)
	{
...
RELEASE_END:
	...
	return ret ; 
#else
	printk(KERN_INFO "%s %s %d open_excl(%d): %p check adb_open %p \n",
		__FILE__, __func__, __LINE__, atomic_read(&_adb_dev->open_excl), adb_fops.release, adb_fops.open);

	if (open_release_pair < 1)
	{
		xlog_printk(ANDROID_LOG_ERROR, XLOG_TAG, "%s %s %d: open_release_pair error\n", __FILE__, __func__, __LINE__);
		return -1;
	}
