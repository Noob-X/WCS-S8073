...
#define USB_WAKE_TIME 5 //wx, seconds for hold wake lock and suspend schedule, please also check musbfsh_core.c to sync, they should be the same
...
static int acm_tty_open(struct tty_struct *tty, struct file *filp)
{
	...
	if (usb_autopm_get_interface(acm->control) < 0)
		...
		//wx, assume modem is capable of remote wakeup to fool autosuspend_check()
...
static bool eint_setup_done = false;
static struct work_struct modem_wakeup_work;
struct wake_lock usb_resume_lock;

static void acm_modem_wakeup_irq()
{
	wake_lock_timeout(&usb_resume_lock, USB_WAKE_TIME * HZ);
	schedule_work(&modem_wakeup_work);
	return;
}

static void modem_wakeup_func(struct work_struct *work)
{
	struct acm *acm = NULL; 
	...
	for(i=0; i<ACM_TTY_MINORS; i++) {
		acm = acm_table[i];
		if(acm) {
			hubdev = acm->dev->parent;
			if(hubdev && hubdev->descriptor.idProduct == 1) { //wx, FIXME, we assume our root hub is the only USB1.1 hub in the system, see the definition of usb11_rh_dev_descriptor			
				break;
			} else {
				acm = NULL;
			}
	...
	if(!acm || !acm->susp_count) {
		return;
	}
	dbg_mtk(&acm->dev->parent->dev, "%s pm_usage_cnt=%d susp_count=%d", __func__, acm->control->pm_usage_cnt, acm->susp_count);
	
	status = usb_autopm_get_interface(acm->control);
	mt65xx_eint_unmask(CUST_EINT_DT_EXT_MD_WK_UP_USB_NUM);
	if(status == 0) {
		usb_autopm_put_interface(acm->control); //wx, to balance the usage count
	}
}
#endif
#endif
...
skip_countries:
...
#ifdef MTK_DT_SUPPORT
#ifdef CONFIG_USB_SUSPEND
	usb_enable_autosuspend(usb_dev); //wx, runtime power management is disabled when running here, due to unknown reason
	pm_runtime_use_autosuspend(&usb_dev->dev); //wx, use interface's specific device
	...
	#if 0 // old EINT ISR design
	if(!eint_setup_done) { 		
		INIT_WORK(&modem_wakeup_work, modem_wakeup_func);
		...
		mt65xx_eint_registration(CUST_EINT_DT_EXT_MD_WK_UP_USB_NUM, CUST_EINT_DT_EXT_MD_WK_UP_USB_DEBOUNCE_EN, CUST_EINT_DT_EXT_MD_WK_UP_USB_POLARITY, acm_modem_wakeup_irq, 0);
		mt65xx_eint_unmask(CUST_EINT_DT_EXT_MD_WK_UP_USB_NUM);
		
		eint_setup_done = true; //wx, acm_probe will be called twice for one CDC-ACM function, due to two interfaces
	}
	#endif
#endif
#endif
...
static int acm_resume(struct usb_interface *intf)
{
	...
	#if 0 // old EINT ISR design
	/*
	   wx, when EINT wakeup a suspended system, driver resume will be called twice, one from runtime pm, one from system pm.
	   The reason we need this workaound here is that, EINT IRQ is processed before system resume procedure. So runtime pm is
	   raised first from EINT IRQ, then system pm will resume acm device again.
	*/
	if(!acm->susp_count) {
		spin_unlock_irq(&acm->read_lock);
		return 0;
