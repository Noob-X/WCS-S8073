...
module_param_named(fake_read, s_fake_read, bool, 0660);
...
static ssize_t logger_fake_read(struct logger_log *log, struct logger_reader *reader, char __user *buf) 
{
        int i;
        char message[960]; // to make sure stack frame not larger than 1024 bytes.
...
start:
	...
	if (unlikely(s_fake_read)) {
		ret = logger_fake_read(log, reader, buf);
		goto out;
	}
