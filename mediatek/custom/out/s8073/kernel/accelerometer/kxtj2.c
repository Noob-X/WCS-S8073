...
static int kxtj2_resume(struct i2c_client *client)
{
	...
	if(err = KXTJ2_SetPowerMode(obj->client, true))	
...
//Ivan added for current consumption issue...
//Ivan FIXME Need to check with real HW: Both EINT3 and EINT4 should be set as input port without any pull up/down???
	mt_set_gpio_dir(GPIO_GSE_2_EINT_PIN, GPIO_DIR_IN);
	mt_set_gpio_mode(GPIO_GSE_2_EINT_PIN, GPIO_MODE_00);
	mt_set_gpio_pull_enable(GPIO_GSE_2_EINT_PIN, FALSE);
	
	mt_set_gpio_dir(GPIO_GSE_1_EINT_PIN, GPIO_DIR_IN);
	mt_set_gpio_mode(GPIO_GSE_1_EINT_PIN, GPIO_MODE_00);
	mt_set_gpio_pull_enable(GPIO_GSE_1_EINT_PIN, FALSE);
