...
VAL_EVENT_T          MT6575_HWLockEvent;    //mutex : HWLockEventTimeoutLock

#define MFLEXVIDEO_DEVNAME     "Vcodec"
#define MT6575_VCODEC_DEV_MAJOR_NUMBER 160   //189

//#define MT6575_MFV_DEBUG
#ifdef MT6575_MFV_DEBUG
#undef MFV_DEBUG
...
static dev_t mflexvideo_devno = MKDEV(MT6575_VCODEC_DEV_MAJOR_NUMBER, 0);
...
static int MT6575Driver_Open_Count;         //mutex : DriverOpenCountLock
...
void MT6575_Video_ISR(VCODEC_IRQ_ENUM a_VcodecIRQ)
{
    ...
    if (grVcodecHWLock.u4VCodecThreadNum == VCODEC_THREAD_MAX_NUM)
    {
        MFV_LOGE("[MT6575_Video_ISR][ERROR] grVcodecHWLock.u4VCodecThreadNum = VCODEC_THREAD_MAX_NUM\n"); 
    ...
    *((volatile VAL_UINT32_T*)oal_hw_context[index].kva_u4HWIsTimeout) = 0;
    ...
    eValRet = eVideoSetEvent(&oal_hw_context[index].MT6575_IsrEvent, sizeof(VAL_EVENT_T));
    spin_unlock_irqrestore(&OalHWContextLock, ulFlags);
    
    if(VAL_RESULT_NO_ERROR != eValRet)
    {
        MFV_LOGE("[MFV][ERROR] ISR set MT6575_IsrEvent error\n");
        //return;
    }            	
    ...
    //MFV_DEBUG("MT6575_Video_ISR-\n");    
}

static irqreturn_t mt6575_video_intr_dlr(int irq, void *dev_id)
{   
    MT6575_Video_ISR(VCODEC_IRQ_VDEC);     //decode
    return IRQ_HANDLED;
}

static irqreturn_t mt6575_video_intr_dlr2(int irq, void *dev_id)
{   
    MT6575_Video_ISR(VCODEC_IRQ_VENC);     //encode
    return IRQ_HANDLED;
}
...
//	pr_info("Vcodec process:%s, pid:%d\n", current->comm, current->pid);
    ...
    case VCODEC_SET_THREAD_ID:
        ...
        if (u4Index == 0xff)
        {
            MFV_LOGE("[ERROR] MT6575_VCODEC_SET_THREAD_ID error, u4Index = %d\n", u4Index);
        }
    ...
    case VCODEC_LOCKHW:
        ...
        while (bLockedHW == VAL_FALSE)
        {
            ...
            if (MT6575_HWLockEvent.u4TimeoutMs == 1)
            {
                MFV_LOGE("[NOT ERROR][VCODEC_LOCKHW] First Use HW!!\n");   
                FirstUseHW = 1;
                eValHWLockRet = eVideoWaitEvent(&MT6575_HWLockEvent, sizeof(VAL_EVENT_T));
                MT6575_HWLockEvent.u4TimeoutMs = 1000;
            }
            else
            {
                FirstUseHW = 0;
            ...
            {
                MFV_LOGE("[ERROR][VCODEC_LOCKHW] MT6575_HWLockEvent TimeOut, CurrentTID = %d\n", current->pid);
                if (FirstUseHW == 1)
                {                    
                }
                else
...
#endif
            {
                ...
                bLockedHW = VAL_TRUE;
                enable_irq(MT6577_VDEC_IRQ_ID);    
    		    enable_irq(MT6577_VENC_IRQ_ID);
            }
    ...
    case VCODEC_INC_PWR_USER:
    	MFV_LOGD("[MT6575] VCODEC_INC_PWR_USER + tid = %d\n", current->pid);
    ...
    case VCODEC_DEC_PWR_USER:
    	MFV_LOGD("[MT6575] VCODEC_DEC_PWR_USER + tid = %d\n", current->pid);
...
#if 0
            //(1) disable IRQ
            disable_irq_nosync(MT6577_VENC_IRQ_ID); 
            disable_irq_nosync(MT6577_VDEC_IRQ_ID);
#endif
            //(2) ack interrupt            
            // decoder
            reg_val = VDO_HW_READ(KVA_VDEC_INT_STA_ADDR);
    ...
    case VCODEC_WAITISR:
        MFV_LOGD("[MT6575] VCODEC_WAITISR + tid = %d\n", current->pid);
        ...
        if (bLockedHW == VAL_FALSE)
        {
            ...
            spin_lock_irqsave(&OalHWContextLock, ulFlags);
            oal_hw_context[index].MT6575_IsrEvent.u4TimeoutMs = 10;
            spin_unlock_irqrestore(&OalHWContextLock, ulFlags);
			//MFV_LOGD(" isrevent timeout =%x\n", oal_hw_context[index].MT6575_IsrEvent.u4TimeoutMs);
			//MFV_LOGD("MT6575_Video_ISR+\n");
            eValRet = eVideoWaitEvent(&oal_hw_context[index].MT6575_IsrEvent, sizeof(VAL_EVENT_T));
    	...
    	//MFV_LOGD(" isrevent timeout =%x\n", oal_hw_context[index].MT6575_IsrEvent.u4TimeoutMs);
    	spin_lock_irqsave(&OalHWContextLock, ulFlags);
    	oal_hw_context[index].MT6575_IsrEvent.u4TimeoutMs = val_isr.u4TimeoutMs;
        spin_unlock_irqrestore(&OalHWContextLock, ulFlags);
		//MFV_LOGD(" isrevent timeout =%x\n", oal_hw_context[index].MT6575_IsrEvent.u4TimeoutMs);
		//MFV_LOGD("MT6575_Video_ISR+\n");
        eValRet = eVideoWaitEvent(&oal_hw_context[index].MT6575_IsrEvent, sizeof(VAL_EVENT_T));
    ...
    case VCODEC_INITHWLOCK:
    {
        ...
        MFV_LOGD("[MT6575] VCODEC_INITHWLOCK + - tid = %d\n", current->pid);
    ...
    case VCODEC_DEINITHWLOCK:    			
    {
        ...
        MFV_LOGD("[MT6575] VCODEC_DEINITHWLOCK + - tid = %d\n", current->pid);
...
static int mflexvideo_open(struct inode *inode, struct file *file)
{
    ...
    MT6575Driver_Open_Count++;

    MFV_LOGE("mflexvideo_open pid = %d, MT6575Driver_Open_Count %d\n", current->pid, MT6575Driver_Open_Count);
...
static int mflexvideo_flush(struct file *file, fl_owner_t id)
{
    ...
    mutex_lock(&DriverOpenCountLock);
    MFV_LOGE("mflexvideo_flush pid = %d, MT6575Driver_Open_Count %d\n", current->pid, MT6575Driver_Open_Count);
	...
	if(MT6575Driver_Open_Count == 0)
	{
		...
		for(i =0; i<VCODEC_MULTIPLE_INSTANCE_NUM; i++)
        {
			...
			// event part
			oal_hw_context[i].MT6575_IsrEvent.pvHandle = "ISR_EVENT";
			oal_hw_context[i].MT6575_IsrEvent.u4HandleSize = sizeof("ISR_EVENT")+1;
			oal_hw_context[i].MT6575_IsrEvent.u4TimeoutMs = 0xFFFFFFFF;  //1000;			
		}
        spin_unlock_irqrestore(&OalHWContextLock, ulFlags);
...
static int MT6575_Vcodec_write(struct file *filp, const char __user *buf, size_t count, loff_t *pos)
{
    return 0;
}

static int MT6575_Vcodec_mmap(struct file *file, struct vm_area_struct *vma)
{
	
    MFV_LOGD("[MT6575_vcodec_mmap] \n");
...
static void MT6575_VCodec_early_suspend(struct early_suspend *h)
{
    mutex_lock(&PWRLock);
    MFV_LOGE("MT6575_VCodec_early_suspend, tid = %d, PWR_USER = %d\n", current->pid, gu4PWRCounter);
    mutex_unlock(&PWRLock);
/*
    if (gu4PWRCounter != 0)
    {
        MFV_LOGE("[MT6575_VCodec_early_suspend] Someone Use HW, Disable Power!\n");
    	disable_clock(MT65XX_PDN_MM_VBUF, "Video_VBUF");
    	disable_clock(MT65XX_PDN_MM_VDEC, "VideoDec");
    	disable_clock(MT65XX_PDN_MM_VENC, "VideoEnc");    			
    	disable_clock(MT65XX_PDN_MM_GDC_SHARE_MACRO, "VideoEnc");
...
*/
    MFV_LOGD("MT6575_VCodec_early_suspend - tid = %d\n", current->pid);
}

static void MT6575_VCodec_late_resume(struct early_suspend *h)
{
    mutex_lock(&PWRLock);
    MFV_LOGE("MT6575_VCodec_late_resume, tid = %d, PWR_USER = %d\n", current->pid, gu4PWRCounter);
    mutex_unlock(&PWRLock);
/*
    if (gu4PWRCounter != 0)
    {
        MFV_LOGE("[MT6575_VCodec_late_resume] Someone Use HW, Enable Power!\n");
    	enable_clock(MT65XX_PDN_MM_VBUF, "Video_VBUF");
    	enable_clock(MT65XX_PDN_MM_VDEC, "VideoDec");
    	enable_clock(MT65XX_PDN_MM_VENC, "VideoEnc");    			
    	enable_clock(MT65XX_PDN_MM_GDC_SHARE_MACRO, "VideoEnc");
...
*/
    MFV_LOGD("MT6575_VCodec_late_resume - tid = %d\n", current->pid);
}

static struct early_suspend vcodec_early_suspend_handler = 
{
	.level = (EARLY_SUSPEND_LEVEL_DISABLE_FB - 1),
	.suspend = MT6575_VCodec_early_suspend,
	.resume = MT6575_VCodec_late_resume,
};
#endif
...
static struct file_operations mflexvideo_fops = {
    ...
    .mmap       = MT6575_Vcodec_mmap,
    .write      = MT6575_Vcodec_write,
};

static int __init mflexvideo_probe(void)
// static int mflexvideo_probe(struct platform_device *dev) 
{
    ...
    {
        pr_info("MFV Device Major number allocated. (%d)\n", ret);
    }
	...
	if(ret < 0)
	{
	pr_info("MFV cdev_add failed");
	}
	else
	{
	pr_info("MFV cdev_add successful");
	}

	mt6577_irq_set_polarity(MT6577_VDEC_IRQ_ID, MT65xx_POLARITY_LOW);
	mt6577_irq_set_polarity(MT6577_VENC_IRQ_ID, MT65xx_POLARITY_LOW);
		
    mt6577_irq_set_sens(MT6577_VDEC_IRQ_ID, MT65xx_LEVEL_SENSITIVE);
    mt6577_irq_set_sens(MT6577_VENC_IRQ_ID, MT65xx_LEVEL_SENSITIVE);
    
    //Register Interrupt 
    if (request_irq(MT6577_VDEC_IRQ_ID , (irq_handler_t)mt6575_video_intr_dlr, 0, MFLEXVIDEO_DEVNAME, NULL) < 0)
    {
    ...
    if (request_irq(MT6577_VENC_IRQ_ID , (irq_handler_t)mt6575_video_intr_dlr2, 0, MFLEXVIDEO_DEVNAME, NULL) < 0)
    {
       MFV_LOGD("[MFV_DEBUG][ERROR] error to request MFlexVideo irq\n"); 
    }
    else
    ...
    disable_irq(MT6577_VDEC_IRQ_ID);
    disable_irq(MT6577_VENC_IRQ_ID);
...
//		mflexvideo_probe(NULL);
	...
	for(i =0; i<VCODEC_MULTIPLE_INSTANCE_NUM; i++)
    {
		...
		oal_hw_context[i].MT6575_IsrEvent.u4TimeoutMs = 0xFFFFFFFF;  //1000;	
		spin_unlock_irqrestore(&OalHWContextLock, ulFlags);
        eValRet = eVideoCreateEvent(&(oal_hw_context[i].MT6575_IsrEvent), sizeof(VAL_EVENT_T));
        if(VAL_RESULT_NO_ERROR != eValRet)
        {
    ...
    //MT6575_HWLockEvent part
    mutex_lock(&HWLockEventTimeoutLock);
    MT6575_HWLockEvent.pvHandle = "HWLOCK_EVENT";
    MT6575_HWLockEvent.u4HandleSize = sizeof("HWLOCK_EVENT")+1;
    MT6575_HWLockEvent.u4TimeoutMs = 1;
    mutex_unlock(&HWLockEventTimeoutLock);
    eValHWLockRet = eVideoCreateEvent(&MT6575_HWLockEvent, sizeof(VAL_EVENT_T));
    if(VAL_RESULT_NO_ERROR != eValHWLockRet)
    {
...
#endif
	pr_info("MFV mflexvideo_driver_init Done\n");    
    return 0;
}
...
static void __exit mflexvideo_driver_exit(void)
{
	...
	free_irq(MT6577_VENC_IRQ_ID, NULL); 
	free_irq(MT6577_VDEC_IRQ_ID, NULL);    
		
    for(i=0; i< OALMEM_STATUS_NUM ; i++)
    {
    	eValRet = eVideoCloseEvent(&(oal_hw_context[i].MT6575_IsrEvent), sizeof(VAL_EVENT_T) );
        if(VAL_RESULT_NO_ERROR != eValRet)
        {
    ...
    //MT6575_HWLockEvent part
    eValHWLockRet = eVideoCloseEvent(&MT6575_HWLockEvent, sizeof(VAL_EVENT_T));
    if(VAL_RESULT_NO_ERROR != eValHWLockRet)
    {
...
MODULE_DESCRIPTION("MT6573 MFlexVideo Driver");
MODULE_LICENSE("GPL");
