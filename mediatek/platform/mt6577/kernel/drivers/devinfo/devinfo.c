...
static int ics_devinfo_proc_read(struct seq_file *m, void *v)
{
	int sec_data = 2;
...
static int ics_devinfo_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, ics_devinfo_proc_read, NULL);
}

static const struct file_operations ics_devinfo_proc_fops = {
	.owner		= THIS_MODULE,
	.open		= ics_devinfo_proc_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
};

static int __init ics_devinfo_proc(void)
{
	pd = proc_mkdir("driver/devinfo", NULL);
	if (!pd) {
		pr_err("[ICS_DEVINFO]: Unable to create proc dir");
		return -ENOMEM;
	}

	pe = proc_create("info", 0, pd, &ics_devinfo_proc_fops);
	if (!pe) {
		pr_err("[ICS_DEVINFO]: Unable to create proc file");
		goto out;
	}

	pr_info("[ICS_DEVINFO]: Proc entry created");

	return 0;
out:
	remove_proc_entry("driver/devinfo", NULL);
	return -ENOMEM;
}
#endif /* ICS_KERNEL */
...
#if ICS_KERNEL
	if (pe)
		remove_proc_entry("info", pd);

	if (pd)
		remove_proc_entry("driver/devinfo", NULL);

	pr_info("[ICS_DEVINFO]: module exit\n");
#else
