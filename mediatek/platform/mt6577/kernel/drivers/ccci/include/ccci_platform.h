...
#define MT_MDWDT_IRQ_LINE 	MT6577_MD_WDT_IRQ_ID
#define MT_DSPWDT_IRQ_LINE  MT6577_MD_WDT_DSP_IRQ_ID
...
#define CCIF_IRQ_CODE MT6577_AP_CCIF_IRQ_ID
...
extern int  ccci_load_firmware(unsigned int load_flag);
...
static inline unsigned int get_chip_version(void)
{
	if(get_chip_sw_ver() == MD_SW_V1)
		...
		return CHIP_E2; // For Fix warning ,should not enter here
...
static inline char *ccci_get_platform_ver(void)
{
	...
	if(get_chip_version() == CHIP_E1)
		platform_ver = "MT6577_S00";
	else if(get_chip_version() == CHIP_E2)
		platform_ver = "MT6577_S01";
	else if(get_chip_version() == CHIP_E3)
		platform_ver = "MT6577_S02";
...
static inline void ccci_get_platform_ver(char * ver)
{
...
{
	#if 0
    mt_irq_set_sens(MT_AP_CCIF_IRQ_ID, MT65xx_LEVEL_SENSITIVE);	
	mt_irq_set_polarity(MT_AP_CCIF_IRQ_ID, MT65xx_POLARITY_LOW);
	#endif
}
