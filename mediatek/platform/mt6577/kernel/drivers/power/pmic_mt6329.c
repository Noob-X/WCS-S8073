...
ssize_t mt6329_read_byte(u8 cmd, u8 *returnData)
{
    ...
    if (ret < 0) 
	{
        PWR_MSG(ANDROID_LOG_WARN, "Power/PMIC", "[mt6329_read_byte] PMIC sends command error!! \n");
        return 0;
    }
    ret = i2c_master_recv(new_client, &readData, 1);
    if (ret < 0) 
	{	
        PWR_MSG(ANDROID_LOG_WARN, "Power/PMIC", "[mt6329_read_byte] PMIC reads data error!! \n");
        return 0;
    } 
    //PWR_MSG(ANDROID_LOG_WARN, "Power/PMIC", "[mt6329_read_byte] Reg[%x]=0x%x \n", cmd, readData);
    *returnData = readData;
#else
    ...
    if (ret < 0) 
	{
        PWR_MSG(ANDROID_LOG_WARN, "Power/PMIC", "[mt6329_read_byte:write&read] PMIC sends command error!! \n");
		new_client->addr = new_client->addr & I2C_MASK_FLAG;
		mutex_unlock(&pmic_i2c_access_mutex_0);
        return 0;
    }
	readData = cmd_buf[0];
    //PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[mt6329_read_byte:write&read] Reg[%x]=0x%x \n", cmd, readData);
    *returnData = readData;
...
int mt6329_multi_read_byte(u8 cmd, u8 *returnData, int len)
{
    ...
    if (ret < 0) 
	{
        PWR_MSG(ANDROID_LOG_WARN, "Power/PMIC", "[mt6329_multi_read_byte] PMIC sends command error!! \n");
        return 0;
    } 
	ret = i2c_master_recv(new_client, readData, len);
    if (ret < 0) {
        PWR_MSG(ANDROID_LOG_WARN, "Power/PMIC", "[mt6329_multi_read_byte] PMIC reads data error!! \n");
        return 0;
    }
...
ssize_t mt6329_write_byte(u8 cmd, u8 writeData)
{
    ...
    if (ret < 0) 
	{
        PWR_MSG(ANDROID_LOG_WARN, "Power/PMIC", "[mt6329_write_byte] sends command error!! \n");
		mutex_unlock(&pmic_i2c_access_mutex_0);
        return 0;
    }
...
int mt6329_multi_write_byte(u8 *cmd, u8 *writeData, int len)
{
	...
	{
		PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[mt6329_mult_write_byte] %x\n", write_data[index]);
	}
	
    ret = i2c_master_send(new_client, write_data, len*2);
    if (ret < 0) 
	{
        PWR_MSG(ANDROID_LOG_WARN, "Power/PMIC", "[mt6329_mult_write_byte] sends command error!! \n");
        return 0;
    }
...
if(g_switch_to_i2c_polling_mode == 0)
{
	...
	//PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[pmic_read_interface] Reg[%x]=0x%x\n", RegNum, pmic6329_reg);
	
	pmic6329_reg &= (MASK << SHIFT);
	*val = (pmic6329_reg >> SHIFT);
	
	//PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[pmic_read_interface] val=0x%x\n", *val);
}
else
{
	//PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "$r");
	//PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[pmic_read_interface] g_switch_to_i2c_polling_mode = 1\r\n");
    ...
    *val = (buffer >> SHIFT);
	...
	//PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[pmic_read_interface] Reg[0x%x]=0x%x, val=0x%x\n", RegNum, buffer, *val);
}
...
{
	mt6329_read_byte(RegNum, &pmic6329_reg);
	//PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[pmic_config_interface] Reg[%x]=0x%x\n", RegNum, pmic6329_reg);
	
	pmic6329_reg &= ~(MASK << SHIFT);
	pmic6329_reg |= (val << SHIFT);

	mt6329_write_byte(RegNum, pmic6329_reg);
	//PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[pmic_config_interface] write Reg[%x]=0x%x\n", RegNum, pmic6329_reg);

	// Check
	//mt6329_read_byte(RegNum, &pmic6329_reg);
	//PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[pmic_config_interface] Check Reg[%x]=0x%x\n", RegNum, pmic6329_reg);
}
else
{
	//PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "$c");
	//PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[pmic_config_interface] g_switch_to_i2c_polling_mode = 1\r\n");
	...
	//buffer = RegNum;
	//result_tmp = mt_i2c_polling_writeread(2,chip_slave_address,&buffer,1,1);
	//PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[pmic_config_interface] Reg[0x%x]=0x%x, val=0x%x\n", RegNum, buffer, pmic6329_reg);
}
...
kal_uint32 pmic_int_en_interface (kal_uint8 RegNum, kal_uint8 sel, kal_bool enable)
{
	...
	mt6329_read_byte(RegNum, &pmic6329_reg);
	//PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[pmic_int_en_interface] Reg[%x]=0x%x\n", RegNum, pmic6329_reg);
	...
	mt6329_write_byte(RegNum, pmic6329_reg);
	//PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[pmic_int_en_interface] write Reg[%x]=0x%x\n", RegNum, pmic6329_reg);
	
	// Check
	mt6329_read_byte(RegNum, &pmic6329_reg);
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[pmic_int_en_interface] Check Reg[%x]=0x%x\n", RegNum, pmic6329_reg);
...
void pmic_get_chip_version_v2(void)
{
    ...
    {
        PWR_MSG(ANDROID_LOG_INFO, "Power/PMIC", "[pmic_get_chip_version_v2] PMIC6326_ECO_1_VERSION !!\n ");
    }
    else if (eco_version == PMIC6326_E2_CID_CODE)
    {
        PWR_MSG(ANDROID_LOG_INFO, "Power/PMIC", "[pmic_get_chip_version_v2] PMIC6326_ECO_2_VERSION !!\n ");
    }
    else if (eco_version == PMIC6326_E3_CID_CODE)
    {
        PWR_MSG(ANDROID_LOG_INFO, "Power/PMIC", "[pmic_get_chip_version_v2] PMIC6326_ECO_3_VERSION !!\n ");
    }
    else if (eco_version == PMIC6326_E4_CID_CODE)
    {
        PWR_MSG(ANDROID_LOG_INFO, "Power/PMIC", "[pmic_get_chip_version_v2] PMIC6326_ECO_4_VERSION !!\n ");
    }
    else
    {
        PWR_MSG(ANDROID_LOG_INFO, "Power/PMIC", "[pmic_get_chip_version_v2] UNKNOWN VERSION !!\n ");
    }
#endif	
...
void pmic_get_chip_version(void)
{
    ...
    if (eco_version == PMIC6326_E1_CID_CODE)
    {
        PWR_MSG(ANDROID_LOG_INFO, "Power/PMIC", "[pmic_get_chip_version] PMIC6326_ECO_1_VERSION !!\n ");
    }
    else if (eco_version == PMIC6326_E2_CID_CODE)
    {
        PWR_MSG(ANDROID_LOG_INFO, "Power/PMIC", "[pmic_get_chip_version] PMIC6326_ECO_2_VERSION !!\n ");
    }
    else if (eco_version == PMIC6326_E3_CID_CODE)
    {
        PWR_MSG(ANDROID_LOG_INFO, "Power/PMIC", "[pmic_get_chip_version] PMIC6326_ECO_3_VERSION !!\n ");
    }
    else if (eco_version == PMIC6326_E4_CID_CODE)
    {
        PWR_MSG(ANDROID_LOG_INFO, "Power/PMIC", "[pmic_get_chip_version] PMIC6326_ECO_4_VERSION !!\n ");
    }
    else
    {
        PWR_MSG(ANDROID_LOG_INFO, "Power/PMIC", "[pmic_get_chip_version] UNKNOWN VERSION !!\n ");
    }
#endif	
...
void pmic_dump_register(void)
{
    ...
    for (i=0;i<PMIC_REG_NUM;i++)
	{
        ...
        PWR_MSG(ANDROID_LOG_INFO, "Power/PMIC", "[pmic_dump_register] Reg[0x%X]=0x%X\n", i, pmic6329_reg[i]);
    }
}
...
{	
	...
	if(temp_val == 0xb)
	{
		PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[pmic_interface_test] pmic_read_interface : PASS\n");
	}
	else
	{
		PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[pmic_interface_test] pmic_read_interface : FAIL(%d)\n", temp_val);
		return 0;
	}
	...
	if(temp_val == 0x3)
	{
		PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[pmic_interface_test] pmic_config_interface : PASS\n");
	}
	else
	{
		PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[pmic_interface_test] pmic_config_interface : FAIL(%d)\n", temp_val);
		return 0;
	}
	...
	if(temp_val == 0x0)
	{
		PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[pmic_interface_test] pmic_int_en_interface : PASS\n");
	}
	else
	{
		PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[pmic_interface_test] pmic_int_en_interface : FAIL(%d)\n", temp_val);
		return 0;
	}
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[pmic_buck_interface_test] \n");
	
	//upmu_buck_csl(BUCK_VPROC, 0x2);

	upmu_buck_vosel(BUCK_VPROC, UPMU_VOLT_0_7_0_0_V);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "##\n");
	upmu_buck_vosel(BUCK_VPROC, UPMU_VOLT_1_0_0_0_V);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "##\n");
	upmu_buck_vosel(BUCK_VPROC, UPMU_VOLT_1_3_0_0_V);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "##\n");

	upmu_buck_vosel(BUCK_VCORE, UPMU_VOLT_0_7_0_0_V);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "##\n");
	upmu_buck_vosel(BUCK_VCORE, UPMU_VOLT_1_0_0_0_V);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "##\n");
	upmu_buck_vosel(BUCK_VCORE, UPMU_VOLT_1_3_0_0_V);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "##\n");

	upmu_buck_vosel(BUCK_VRF18, UPMU_VOLT_1_5_0_0_V);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "##\n");
	upmu_buck_vosel(BUCK_VRF18, UPMU_VOLT_1_9_0_0_V);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "##\n");
	upmu_buck_vosel(BUCK_VRF18, UPMU_VOLT_2_1_2_0_V);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "##\n");

	upmu_buck_vosel(BUCK_VIO18, UPMU_VOLT_1_5_0_0_V);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "##\n");
	upmu_buck_vosel(BUCK_VIO18, UPMU_VOLT_1_9_0_0_V);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "##\n");
	upmu_buck_vosel(BUCK_VIO18, UPMU_VOLT_2_1_2_0_V);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "##\n");

	upmu_buck_vosel(BUCK_VPA, UPMU_VOLT_0_9_0_0_V);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "##\n");
	upmu_buck_vosel(BUCK_VPA, UPMU_VOLT_2_1_0_0_V);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "##\n");
	upmu_buck_vosel(BUCK_VPA, UPMU_VOLT_3_4_0_0_V);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "##\n");
	
}

void pmic_ldo_interface_test(void)
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC","[pmic_ldo_interface_test] \n");
	
	upmu_ldo_en(LDO_VM12_1		  , 0x1);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC","##\n");
	upmu_ldo_en(LDO_VM12_2		  , 0x1);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC","##\n");
	upmu_ldo_en(LDO_VM12_INT	  , 0x1);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC","##\n");
	upmu_ldo_en(LDO_VIO28		  , 0x1);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC","##\n");
	upmu_ldo_en(LDO_VSIM		  , 0x1);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC","##\n");
	upmu_ldo_en(LDO_VSIM2		  , 0x1);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC","##\n");
	upmu_ldo_en(LDO_VUSB		  , 0x1);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC","##\n");
	upmu_ldo_en(LDO_VCAMD		  , 0x1);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC","##\n");
	upmu_ldo_en(LDO_VCAM_IO 	  , 0x1);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC","##\n");
	upmu_ldo_en(LDO_VCAM_AF 	  , 0x1);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC","##\n");
	upmu_ldo_en(LDO_VMC 		  , 0x1);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC","##\n");
	upmu_ldo_en(LDO_VMCH		  , 0x1);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC","##\n");
	upmu_ldo_en(LDO_VGP 		  , 0x1);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC","##\n");
	upmu_ldo_en(LDO_VGP2		  , 0x1);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC","##\n");
	upmu_ldo_en(LDO_VIBR		  , 0x1);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC","##\n");
	upmu_ldo_en(LDO_VRF 		  , 0x1);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC","##\n");
	upmu_ldo_en(LDO_VTCXO		  , 0x1);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC","##\n");
	upmu_ldo_en(LDO_VA1 		  , 0x1);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC","##\n");
	upmu_ldo_en(LDO_VA2 		  , 0x1);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC","##\n");
	upmu_ldo_en(LDO_VCAMA		  , 0x1);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC","##\n");
	upmu_ldo_en(LDO_VRTC		  , 0x1);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC","##\n");

	upmu_ldo_vosel(LDO_VSIM, UPMU_VOLT_3_0_0_0_V);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC","##\n");
	upmu_ldo_vosel(LDO_VSIM2, UPMU_VOLT_3_0_0_0_V);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC","##\n");
	upmu_ldo_vosel(LDO_VCAMD, UPMU_VOLT_2_5_0_0_V);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC","##\n");
	upmu_ldo_vosel(LDO_VCAM_IO, UPMU_VOLT_1_8_0_0_V);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC","##\n");
	upmu_ldo_vosel(LDO_VCAM_AF, UPMU_VOLT_2_8_0_0_V);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC","##\n");
	upmu_ldo_vosel(LDO_VMC, UPMU_VOLT_1_5_0_0_V);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC","##\n");
	upmu_ldo_vosel(LDO_VMCH, UPMU_VOLT_1_8_0_0_V);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC","##\n");
	upmu_ldo_vosel(LDO_VGP, UPMU_VOLT_1_8_0_0_V);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC","##\n");
	upmu_ldo_vosel(LDO_VGP2, UPMU_VOLT_2_8_0_0_V);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC","##\n");
	upmu_ldo_vosel(LDO_VIBR, UPMU_VOLT_3_3_0_0_V);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC","##\n");
	upmu_ldo_vosel(LDO_VA1, UPMU_VOLT_2_1_0_0_V);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC","##\n");
	upmu_ldo_vosel(LDO_VA2, UPMU_VOLT_2_8_0_0_V);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC","##\n");
	upmu_ldo_vosel(LDO_VCAMA, UPMU_VOLT_2_5_0_0_V);PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC","##\n");
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[pmic_auxadc_test] \n");
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[pmic_chr_test] \n");
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[dct_pmic_VM12_1_enable] %d\n", dctEnable);
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[dct_pmic_VM12_2_enable] %d\n", dctEnable);
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[dct_pmic_VM12_INT_enable] %d\n", dctEnable);
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[dct_pmic_VIO28_enable] %d\n", dctEnable);
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[dct_pmic_VSIM_enable] %d\n", dctEnable);
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[dct_pmic_VSIM2_enable] %d\n", dctEnable);
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[dct_pmic_VUSB_enable] %d\n", dctEnable);
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[dct_pmic_VCAMD_enable] %d\n", dctEnable);
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[dct_pmic_VCAM_IO_enable] %d\n", dctEnable);
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[dct_pmic_VCAM_AF_enable] %d\n", dctEnable);
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[dct_pmic_VMC_enable] %d\n", dctEnable);
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[dct_pmic_VMCH_enable] %d\n", dctEnable);
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[dct_pmic_VGP_enable] %d\n", dctEnable);
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[dct_pmic_VGP2_enable] %d\n", dctEnable);
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[dct_pmic_VIBR_enable] %d\n", dctEnable);
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[dct_pmic_VRF_enable] %d\n", dctEnable);
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[dct_pmic_VTCXO_enable] %d\n", dctEnable);
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[dct_pmic_VA1_enable] %d\n", dctEnable);
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[dct_pmic_VA2_enable] %d\n", dctEnable);
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[dct_pmic_VCAMA_enable] %d\n", dctEnable);
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[dct_pmic_VRTC_enable] %d\n", dctEnable);
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[dct_pmic_VM12_1_sel] No vlotage can setting on E1 !\n");
}

void dct_pmic_VM12_2_sel(kal_uint32 volt)
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[dct_pmic_VM12_2_sel] No vlotage can setting on E1 !\n");
}

void dct_pmic_VM12_INT_sel(kal_uint32 volt)
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[dct_pmic_VM12_INT_sel] No vlotage can setting on E1 !\n");
}

void dct_pmic_VIO28_sel(kal_uint32 volt)
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[dct_pmic_VIO28_sel] No vlotage can setting on E1 !\n");
}

void dct_pmic_VSIM_sel(kal_uint32 volt)
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "****[dct_pmic_VSIM_sel] value=%d \n", volt);
	...
	if(volt == VOL_DEFAULT)	{
		...
		PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "Error Setting %d. DO nothing.\r\n", volt);
	}
}

void dct_pmic_VSIM2_sel(kal_uint32 volt)
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "****[dct_pmic_VSIM2_sel] value=%d \n", volt);
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[dct_pmic_VUSB_sel] No vlotage can setting on E1 !\n");
}

void dct_pmic_VCAMD_sel(kal_uint32 volt)
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "****[dct_pmic_VCAMD_sel] value=%d \n", volt);
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "****[dct_pmic_VCAM_IO_sel] value=%d \n", volt);
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "****[dct_pmic_VCAM_AF_sel] value=%d \n", volt);
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "****[dct_pmic_VMC_sel] value=%d \n", volt);
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "****[dct_pmic_LDO_VMCH_sel] value=%d \n", volt);
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "****[dct_pmic_VGP_sel] value=%d \n", volt);
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "****[dct_pmic_VGP2_sel] value=%d \n", volt);
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "****[dct_pmic_VIBR_sel] value=%d \n", volt);
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[dct_pmic_VRF_sel] No vlotage can setting on E1 !\n");
}

void dct_pmic_VTCXO_sel(kal_uint32 volt)
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[dct_pmic_VTCXO_sel] No vlotage can setting on E1 !\n");
}

void dct_pmic_VA1_sel(kal_uint32 volt)
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "****[dct_pmic_VA1_sel] value=%d \n", volt);
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "****[dct_pmic_VA2_sel] value=%d \n", volt);
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "****[dct_pmic_VCAMA_sel] value=%d \n", volt);
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[dct_pmic_VRTC_sel] No vlotage can setting on E1 !\n");
}
...
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[pmic_ldo_enable] Receive powerId %d, action is %d\n", powerId, powerEnable);
	...
	{
		PWR_MSG(ANDROID_LOG_WARN, "Power/PMIC", "[pmic_ldo_enable] UnKnown powerId (%d)\n", powerId);
	}		
}

void pmic_ldo_vol_sel(MT65XX_POWER powerId, MT65XX_POWER_VOLTAGE powerVolt)
{
	PWR_MSG(ANDROID_LOG_DEBUG, "Power/PMIC", "[pmic_ldo_vol_sel] Receive powerId %d, action is %d\n", powerId, powerVolt);
	...
	{
		PWR_MSG(ANDROID_LOG_WARN, "Power/PMIC", "[pmic_ldo_ldo_vol_sel] UnKnown powerId (%d)\n", powerId);
	}
}
...
{         
	...
	PWR_MSG(ANDROID_LOG_INFO, "Power/PMIC", "[mt6329_driver_detect] \n");
