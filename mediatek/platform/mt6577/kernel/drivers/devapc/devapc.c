...
static int __init devapc_init(void)
{
     ...
     */
    ret = request_irq(MT6577_APARM_DOMAIN_IRQ_ID, (irq_handler_t)devapc_violation_irq, IRQF_TRIGGER_LOW | IRQF_SHARED, 
        "mt6577_devapc", &g_devapc_ctrl);    
    disable_irq(MT6577_APARM_DOMAIN_IRQ_ID);
    enable_irq(MT6577_APARM_DOMAIN_IRQ_ID);
...
static void __exit devapc_exit(void)
{
    ...
    free_irq(MT6577_APARM_DOMAIN_IRQ_ID ,  g_devapc_ctrl);   
